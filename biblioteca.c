...
int carregarClientes(Cliente clientes[]) {
  ...
  if (arquivo) { // Verifica se o arquivo foi aberto com sucesso
    fread(clientes, sizeof(Cliente), 1000,
          arquivo);              // Lê os dados dos clientes do arquivo
    fseek(arquivo, 0, SEEK_END); // Move o cursor para o fim do arquivo
    numClientes =
        ftell(arquivo) /
        sizeof(Cliente); // Calcula o número de clientes com base no tamanho do
                         // arquivo e o tamanho de cada registro de cliente
    fclose(arquivo);     // Fecha o arquivo
  }
...
void salvarExtrato(Transacao extratoClientes[], int numExtratos) {
  FILE *arquivo = fopen("extratos_bancarios.bin",
                        "wb"); // Abre um arquivo binário para escrita
  if (arquivo) {               // Verifica se o arquivo foi aberto com sucesso
    fwrite(extratoClientes, sizeof(Transacao), numExtratos,
           arquivo); // Escreve os dados de transação no arquivo
    fclose(arquivo); // Fecha o arquivo
  } else {
    printf(
        "Erro ao abrir o arquivo de extratos para gravação.\n"); // Imprime uma
                                                                 // mensagem de
                                                                 // erro caso o
                                                                 // arquivo não
                                                                 // possa ser
...
int carregarExtrato(Transacao extratoClientes[]) {
  ...
  if (arquivo) { // Verifica se o arquivo foi aberto com sucesso
    fread(extratoClientes, sizeof(Transacao), 1000,
          arquivo);              // Lê os dados de transação do arquivo
    fseek(arquivo, 0, SEEK_END); // Move o cursor para o fim do arquivo
    numExtratos =
        ftell(arquivo) /
        sizeof(
            Transacao); // Calcula o número de transações com base no tamanho do
                        // arquivo e o tamanho de cada registro de transação
    fclose(arquivo); // Fecha o arquivo
  }
...
void debito(Cliente clientes[], int numClientes) {
    ...
    // Solicita ao usuário que digite os dados do débito
    printf("Digite o CPF: ");
    scanf("%s", cpf);
    ...
    if (clienteEncontrado != -1) {
        ...
        if (clientes[clienteEncontrado].tipo_conta == 1) {
            if (clientes[clienteEncontrado].saldo - valorDebito >= -1000) {
                ...
                strcpy(clientes[clienteEncontrado].extrato[clientes[clienteEncontrado].numTransacoes].data, data);
                strcpy(clientes[clienteEncontrado].extrato[clientes[clienteEncontrado].numTransacoes].hora, hora);
                clientes[clienteEncontrado].numTransacoes++;
                printf("Débito de %.2f realizado com sucesso.\n", valorDebito);
            } else {
                printf("Limite de saldo negativo excedido para a conta comum. Não é possível realizar o débito.\n");
            }
        }
        // Se a conta for Plus e o saldo ficar abaixo de -5000, restringir o débito
        else if (clientes[clienteEncontrado].tipo_conta == 2) {
            if (clientes[clienteEncontrado].saldo - valorDebito >= -5000) {
                // Aplica a taxa de débito do tipo Plus de 3%
                valorDebito *= 1.03;
                clientes[clienteEncontrado].saldo -= valorDebito;
                // Atualize o extrato do cliente
                clientes[clienteEncontrado].extrato[clientes[clienteEncontrado].numTransacoes].tipo = 'D';
                ...
                printf("Limite de saldo negativo excedido para a conta Plus. Não é possível realizar o débito.\n");
            }
        }
    } else {
...
void deposito(Cliente clientes[], int numClientes) {
  ...
  if (clienteEncontrado != -1) {
    ...
    clientes[clienteEncontrado]
        .extrato[clientes[clienteEncontrado].numTransacoes]
        .valor = valorDeposito;
    strcpy(clientes[clienteEncontrado]
               .extrato[clientes[clienteEncontrado].numTransacoes]
               .data,
           data);
    strcpy(clientes[clienteEncontrado]
  ...
  if (clienteEncontrado != -1) {
    ...
    for (int i = 0; i < clientes[clienteEncontrado].numTransacoes; i++) {
      if (clientes[clienteEncontrado].extrato[i].tipo == 'D') {
        printf("(-) Débito: %.2lf - Data: %s Hora: %s\n ",
               clientes[clienteEncontrado].extrato[i].valor,
               clientes[clienteEncontrado].extrato[i].data,
               clientes[clienteEncontrado].extrato[i].hora);
      } else if (clientes[clienteEncontrado].extrato[i].tipo == 'C') {
        printf("(+) Depósito: %.2lf - Data: %s Hora: %s\n ",
               clientes[clienteEncontrado].extrato[i].valor,
               clientes[clienteEncontrado].extrato[i].data,
               clientes[clienteEncontrado].extrato[i].hora);
      } else if (clientes[clienteEncontrado].extrato[i].tipo == 'P') {
        printf("(-) Transferência: %.2lf - Data: %s Hora: %s\n ",
               clientes[clienteEncontrado].extrato[i].valor,
               clientes[clienteEncontrado].extrato[i].data,
               clientes[clienteEncontrado].extrato[i].hora);
    ...
    if (clientes[clienteOrigem].tipo_conta == 1) {
      if (clientes[clienteOrigem].saldo - valorTransferencia >= -1000) {
        ...
        clientes[clienteDestino]
            .extrato[clientes[clienteDestino].numTransacoes]
            .valor = valorTransferencia;
        strcpy(clientes[clienteDestino]
                   .extrato[clientes[clienteDestino].numTransacoes]
                   .data,
               data);
        strcpy(clientes[clienteDestino]
